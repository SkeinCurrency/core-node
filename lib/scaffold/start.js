...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 ...
                 'There are changes necessary in both dash.conf and core-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/skeincurrency/core-node/blob/dashd/docs/upgrade.md \n');
 ...
 * @param {Object} options.config - The parsed core-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './core-node.json');
  ...
  node.on('ready', function() {
    log.info('SkeinCurrency Node ready');
  });
...
function loadModule(req, service) {
  try {
    // first try in the built-in core-node services directory
 ...
 * @param {Node} node - The SkeinCurrency Node instance
 */
