...
var Proposal = skeincore.GovObject.Proposal;
...
var rawtxTopicBuffer = Buffer.from('7261777478', 'hex');
var rawtxlockTopicBuffer = Buffer.from('72617774786c6f636b', 'hex');
/**
...
Dash.prototype.subscribeAddress = function(emitter, addresses) {
  ...
  if(addresses){
    for(var i = 0; i < addresses.length; i++) {
      if (skeincore.Address.isValid(addresses[i], this.node.network)) {
...
Dash.prototype._loadSpawnConfiguration = function(node) {
  ...
  log.info('Using SkeinCurrency config file:', configPath);
...
Dash.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    ...
    '"txindex" option is required in order to use transaction query features of core-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of core-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of core-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to dashd from skeincore. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that dashd is undergoing a reindex. ' +
             ...
             'of core-node services will start.');
    node._reindex = true;
  }
};
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('SkeinCurrency Daemon Ready');
          callback();
        });
      });
...
Dash.prototype._getDefaultConf = function() {
  ...
  if (this.node.network === skeincore.Networks.testnet) {
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('SkeinCurrency Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Dash.prototype._zmqTransactionHandler = function(node, message) {
  ...
  var self = this;
  var hash = skeincore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    var txid = skeincore.util.buffer.reverse(hash).toString('hex');
...
Dash.prototype._checkReindex = function(node, callback) {
  ...
  interval = setInterval(function() {
    node.client.getBlockchainInfo(function(err, response) {
      ...
      log.info('SkeinCurrency Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
...
Dash.prototype._stopSpawnedDash = function(callback) {
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned SkeinCurrency process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean SkeinCurrency process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Dash.prototype._spawnChildProcess = function(callback) {
  ...
  self._stopSpawnedDash(function(err) {
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('SkeinCurrency process unexpectedly exited with code:', code);
        ...
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            ...
            log.warn('SkeinCurrency process restarted');
          });
...
Dash.prototype.start = function(callback) {
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('SkeinCurrency configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {String|Address|Array} addressArg - An address string, skeincore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
...
Dash.prototype.getAddressUnspentOutputsPaginated = function(addressArg, options, callback) {
  ...
  function transformUnspentOutput(delta) {
    var script = skeincore.Script.fromAddress(delta.address);
    return {
...
Dash.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    ...
    if (clone) {
      // reverse start and end as the order in skeincore is most recent to less recent
 ...
 * @param {Object} txid - A SkeinCurrency transaction id
 * @param {Function} callback
 */
...
Dash.prototype._getAddressStrings = function(addresses) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof skeincore.Address) {
...
/**
 * Will retrieve a block as a SkeinCurrency object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = skeincore.Block.fromString(response.result);
...
/**
 * Will get a transaction as a SkeinCurrency Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
